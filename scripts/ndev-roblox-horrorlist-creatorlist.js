...
async function fetchData() {
    ...
    const creatorRatings = new Map();

    function addRating(creator, rating) {
        if (creatorRatings.has(creator)) {
            const ratings = creatorRatings.get(creator);
            ratings.push(rating);
        } else {
            creatorRatings.set(creator, [rating]);
        }
    }

    for (let i = 0; i < gameUIDS.length; i++) {
        try {
            addRating(gameDataFromAPI[i].creator.name, data.databaseData.games[i].ratings.rating);
        } catch (e) {
            console.error(e);
        }
    }
    ...
    creatorRatings.forEach((value, key) => {
        const ratings = value.map(Number);
        const sum = ratings.reduce((total, rating) => total + rating, 0);
        const averageRating = sum / ratings.length;

        sortedRatings.push({ creator: key, averageRating });
    });

    sortedRatings.sort((a, b) => b.averageRating - a.averageRating);

    for (let i = 0; i < sortedRatings.length; i++) {
        try {
            var row = ` <tr class="hover-reveal">
                  <td data-th="Placement">${i + 1}.</td>
                  <td data-th="Creator" class="align-left">${sortedRatings[i].creator}</td>
                  <td data-th="Rating" style="text-align: right;">${sortedRatings[i].averageRating.toFixed(1)}</td>
                  </tr>`;
